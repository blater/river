...
| U02b: variable-width rows and text | `VARCHAR(n)`, for `1 <= n <= 255`, stores strict UTF-8 and works through heap/index, constraints, sort/spill, recovery, backup, protocol, JDBC, and CLI. | `n` counts Unicode scalar values; one value is at most 1,020 encoded bytes and a table's declared worst-case row must fit the existing 4 KiB row bound. No Unicode normalization is implicit. V1 has one deterministic, case-sensitive Unicode-code-point collation. The current packed `VARCHAR(7)` representation is removed as SQL-visible storage or retained only as a semantics-equivalent private optimization. |
| U02c: boolean and exact decimal | `BOOLEAN` and `DECIMAL(p,s)`, with `1 <= p <= 18` and `0 <= s <= p`, work in DDL, literals, parameters, casts, `WHERE`, `JOIN ... ON`, `HAVING`, `CHECK`, `BETWEEN`, `IN`, correlated predicates, indexes, grouping/distinct, aggregates, defaults, and recovery. JDBC exposes decimal values as `BigDecimal`. | Decimal values use a signed scaled 64-bit integer internally and reusable wide intermediates for rescaling/math. Implement unary sign, `+`, `-`, `*`, `/`, `%`, `ABS`, `CEIL`, `FLOOR`, `ROUND`, and `TRUNCATE`; `SUM`, `AVG`, `MIN`, and `MAX`; exact cross-scale comparison; deterministic result precision/scale and half-even division/rounding; SQLSTATE `22012` for division by zero; and checked `22003` overflow. Prove `HAVING` over decimal aggregates and `BETWEEN`/range-index equivalence. No floating-point type, per-row `BigDecimal`, or implicit lossy numeric conversion enters v1. |
...
- Real SQL proves signed `Long.MIN_VALUE`, zero, and `Long.MAX_VALUE` primary
  keys plus unique full-domain `BIGINT`, `DECIMAL(18,1)`, local timestamp, and
...
- Bounded postfix projections now compose at compile time through selected
  ...
  256 MiB each. It preserves the projection-only point/index fast path,
