...
data-directory-owned PID record and never searches for or terminates an unrelated
PID.
...
- `riverd --help`, `riverd help start`, `riverd help stop`, and a command's
  `--help` option print the relevant comprehensive reference;
...
writes outside the resolved data directory except an explicitly supplied
`--ready-file`.
