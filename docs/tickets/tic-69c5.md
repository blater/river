...
parent: tic-c8d1
---
# Simplify LockDeadlockDiagnosticsSnapshot

File: `river-tx/src/main/java/io/riverdb/tx/LockDeadlockDiagnosticsSnapshot.java`. Baseline slopwatch score: **126.216**.

## Approach

Review `LockDeadlockDiagnosticsSnapshot.validForDiagnosticGate`, `LockDeadlockDiagnosticsSnapshot.exemplarEdgeIndex`, `LockDeadlockDiagnosticsSnapshot.copyFrom` first. Separate their distinct validation, execution and cleanup responsibilities into concrete local operations; flatten status-dependent control flow while preserving ordering and ownership. Reuse an existing owner where one exists, and avoid new delegation layers that merely move branches.
