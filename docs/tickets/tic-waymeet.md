...
### 2026-09-16T09:11:54Z

Implemented the requested namespace-first finite-key comparison in BTreePage.childForKey (one production method). Each probe reads the separator namespace first and reads the key only when namespaces match; routing no longer invokes the general infinity-aware comparator. Independent caller audit confirmed finite search namespaces for point, mutation, snapshot, scan-lower-bound and vacuum paths; persisted separators are finite, while infinity remains an upper/page fence handled by the unchanged general comparator. Equality routes right and signed extremes remain correct. Existing exhaustive routing tests cover all occupancies, namespaces, gaps, equality, signed extremes and read-only buffers. Validation: :river-storage:check plus :river-engine:test --tests io.riverdb.engine.table.IndexedTransactionSessionTest, successful in 8s; counts {'river-storage': {'tests': 49, 'failures': 0, 'errors': 0, 'skipped': 0}, 'river-engine': {'tests': 49, 'failures': 0, 'errors': 0, 'skipped': 0}}; log /private/tmp/river-waymeet-namespace-check.log. Slopmark BTreePage 28.4196→28.5683, no new responsibility or allocation. Earlier performance samples apply to df9cb264 before this refinement; no new performance result is claimed. Candidate remains on ticket/tic-waymeet-recheck and unmerged.
