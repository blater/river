...
final class TpccReport {
  ...
  static void results(TpccConfig config, TpccMetrics metrics) {
    ...
    System.out.println("whole_transaction_retries=" + metrics.retry().retries());
    ...
    System.out.println("drain_unclassified_retry_failures="
        + metrics.retry().drainUnclassifiedRetryFailures());
    System.out.println("retry_correlation_overflows=" + metrics.retry().retryCorrelationOverflows());
    System.out.println("metrics_overflowed=" + metrics.overflowed());
    System.out.println("server_retryable_outcomes=" + metrics.retry().retryableOutcomes());
    System.out.println("drain_server_retryable_outcomes=" + metrics.retry().drainRetryableOutcomes());
    ...
    System.out.println("maximum_latency_us=" + metrics.latency().maximumLatencyMicros());
    System.out.println("protocol_requests=" + metrics.protocol().requests());
    System.out.println("logical_exchanges=" + metrics.protocol().requests()
    ...
    System.out.println("protocol_bytes_sent=" + metrics.protocol().bytesSent());
    System.out.println("protocol_bytes_received=" + metrics.protocol().bytesReceived());
    System.out.println("drain_protocol_requests=" + metrics.protocol().drainRequests());
    System.out.println("drain_protocol_bytes_sent=" + metrics.protocol().drainBytesSent());
    System.out.println("drain_protocol_bytes_received=" + metrics.protocol().drainBytesReceived());
    System.out.println("protocol_requests_per_commit=" + String.format(
        java.util.Locale.ROOT, "%.1f",
        metrics.totalCommitted() == 0
            ? 0.0 : metrics.protocol().requests() / (double) metrics.totalCommitted()));
    System.out.println("protocol_requests_per_transaction_attempt=" + String.format(
        java.util.Locale.ROOT, "%.1f",
        metrics.retry().transactionAttempts() == 0
    ...
    for (StatusCode status : StatusCode.values()) {
      ...
      long drainOutcomes = metrics.retry().drainRetryableOutcomes(status);
      long drainRetries = metrics.retry().drainClientRetries(status);
    ...
    for (TpccTransactionType type : TpccTransactionType.values()) {
      System.out.println("transaction=" + type + " committed=" + metrics.committed(type)
          ...
          + " retries=" + metrics.retry().retries(type)
          ...
          + " protocol_requests=" + metrics.protocol().requests(type)
          + " drain_protocol_requests=" + metrics.protocol().drainRequests(type)
          ...
          + " p50_us_upper_bound=" + metrics.latency().percentileMicros(type, 50)
          + " p95_us_upper_bound=" + metrics.latency().percentileMicros(type, 95)
          + " p99_us_upper_bound=" + metrics.latency().percentileMicros(type, 99)
          + " p99_9_us_upper_bound=" + metrics.latency().percentileMicrosPermille(type, 999)
          + " maximum_us=" + metrics.latency().maximumLatencyMicros(type));
      for (StatusCode status : StatusCode.values()) {
        long outcomes = metrics.retry().retryableOutcomes(type, status);
        long clientRetries = metrics.retry().clientRetries(type, status);
        long drainOutcomes = metrics.retry().drainRetryableOutcomes(type, status);
        long drainRetries = metrics.retry().drainClientRetries(type, status);
        if (outcomes != 0 || clientRetries != 0 || drainOutcomes != 0 || drainRetries != 0) {
