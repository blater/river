...
final class SqlQueryExecutionResourceCleanup {
  ...
  static StatusCode close(
    ...
    StatusCode status = closeStatementResources(
        universalJoins, descriptorScans, unions, catalogs);
    status = closeQueryResources(
        status, session, subqueries, joins, pointQueries, activeScan, sorts, blockPipeline);
    status = firstFailure(status, groups.closeResources());
    return status;
  }

  private static StatusCode closeStatementResources(
      SqlUniversalJoinExecution universalJoins,
      SqlDescriptorScanExecution descriptorScans,
      SqlUnionSessionExecution unions,
      SqlCatalogScanExecution catalogs) {
    ...
    status = firstFailure(status, descriptorScans.active() || descriptorScans.matched()
        ? descriptorScans.close() : StatusCode.OK);
    status = firstFailure(status, unions.active() ? unions.close() : StatusCode.OK);
    return firstFailure(status, catalogs.close());
  }

  private static StatusCode closeQueryResources(
      StatusCode status,
      RelationalSession session,
      SqlSubqueryGraphExecution subqueries,
      SqlJoinExecution joins,
      ...
      SqlSortExecution sorts,
      SqlBlockPipelineExecution blockPipeline) {
    if (subqueries.hasResources()) status = firstFailure(status, subqueries.reset());
    if (joins.hasResources()) status = firstFailure(status, joins.close());
    ...
    if (sorts.hasResources()) status = firstFailure(status, sorts.close());
