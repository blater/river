...
public final class SqlSession {
  ...
  private StatusCode beginCatalogObjectScan(SqlScanCursor cursor) {
    ...
    if (status.isOk()) {
      status = session.beginCatalogObjectScan(cursor.catalogObjects());
    }
    if (status.isOk()) {
      status = cursor.claimCatalogObjects(this, implicit);
    }
    if (status.isOk()) {
      scanActive = true;
      return StatusCode.OK;
    }
    if (cursor.catalogObjects().isActive()) {
      session.closeCatalogObjectScan(cursor.catalogObjects());
    }
    return failScanStart(status, cursor, implicit);
  }
  ...
  private void describePlan(SqlScanCursor cursor) {
    ...
    if (command.type() == SqlCommandType.JOIN_SCAN) {
      addPlanStep(
          cursor != null && cursor.joinInnerIndexed()
              ? PLAN_LOOKUP : PLAN_TABLE,
          cursor == null ? -1 : cursor.joinInnerColumn());
    }
  }
  ...
  public StatusCode nextScan(SqlScanCursor cursor, SqlScanRowResult result) {
    ...
    if (cursor.catalogObjectScan()) {
      StatusCode status = session.nextCatalogObject(
          cursor.catalogObjects(), catalogObject);
      if (!status.isOk()) {
        return status;
      }
  ...
  private StatusCode nextJoin(SqlScanCursor cursor, SqlScanRowResult result) {
    while (true) {
      if (cursor.joinInnerScanActive()) {
        ...
        if (cursor.joinInnerIndexed()) {
          ...
          inner = session.nextScan(
              cursor.joinInnerRelational(), aggregateRow);
        ...
        if (!cursor.joinInnerIndexed()
            && (isNull(innerRow, joinTable, cursor.joinInnerColumn())
                || readColumn(innerKey, innerRow, cursor.joinInnerColumn())
      ...
      if (!cursor.joinInnerUnique()) {
        ...
        cursor.beginJoinInnerScan(outerKey, joinValue);
        continue;
      }
