...
final class SqlSessionExecutionCoordinator {
  ...
  StatusCode validatePrepared(
    ...
    if (status.isOk()) {
      status = authorize(reuse ? candidate.template().type() : bound.command.type());
    }
    boolean began = false;
    if (status.isOk()) {
    ...
    // Recheck under the existing schema admission: DDL may have committed before begin.
    ...
    if (status.isOk() && !reuse && bound.command.tableName().length() > 0) {
    ...
    if (status.isOk() && reuse) result.reuse(candidate);
      ...
      status = capturePrepared(budget, result, generation, preparationGeneration);
    }
    if (parsed) bound.reset();
    return status;
  }

  private StatusCode capturePrepared(
