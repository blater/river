...
final class IndexedGroupCommitMetrics {
  ...
  synchronized void recordGroupLockHoldingsReleased(LockScope scope, long count) {
    values.recordGroupLockHoldingsReleased(scope, count);
    if (capturing) capture.recordGroupLockHoldingsReleased(scope, count);
  }
