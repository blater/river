...
final class EmbeddedRiverDropIndexTest {
  private static final DatabaseIncarnation DATABASE =
      DatabaseIncarnation.of(0x44524f50494e4458L, 0x454e47494e453031L);
  private static final WalGeneration GENERATION = WalGeneration.of(1);

  @Test
  void dropsIndexStorageAndCatalogThenReusesTheName(@TempDir Path root) {
    ...
    assertEquals(
        StatusCode.OK,
        session.execute("CREATE TABLE other (id BIGINT PRIMARY KEY, value BIGINT)", result));
    assertEquals(
        StatusCode.OK,
        session.execute(
    ...
    assertEquals(
        StatusCode.RETRY,
        session.execute("DROP INDEX items_category ON items", result));
    assertEquals(StatusCode.OK, second.execute("ROLLBACK", result));
    assertEquals(StatusCode.OK, second.close());

    assertEquals(
        StatusCode.CONFLICT,
        session.execute("DROP INDEX items_category ON other", result));
