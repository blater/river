...
final class EmbeddedRiverRenameIndexTest {
  private static final DatabaseIncarnation DATABASE =
      DatabaseIncarnation.of(0x52454e414d45494eL, 0x4445583030303031L);
  private static final WalGeneration GENERATION = WalGeneration.of(1);

  @Test
  void renamesIndexIdentityWithoutRebuildingStorage(@TempDir Path root) {
    ...
    assertEquals(
        StatusCode.CONFLICT,
        session.execute("ALTER INDEX items_sku RENAME TO unavailable", result));
    ...
    assertEquals(
        StatusCode.CONFLICT,
        session.execute("ALTER INDEX items_code RENAME TO unavailable", result));
    ...
    assertEquals(
        StatusCode.OK,
        session.execute("DROP INDEX items_sku ON items", result));
    assertEquals(
        StatusCode.OK,
        session.execute("INSERT INTO items VALUES (3, 'alpha', 8)", result));
    assertEquals(2, countRows(session, "SELECT id FROM items WHERE category=7"));
