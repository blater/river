...
cleanup() {
  ...
  # Database contents and generated credentials belong to the ephemeral managed server,
  # even when diagnostic output is retained. Never preserve them as report artifacts.
  if [[ $owned_process_cleanup_valid == true ]] && ! rm -rf -- "$temp_dir/database"; then
...
client_file=$(property server_client_config "$server_log")
...
client_java_options=( "--enable-native-access=ALL-UNNAMED" "${client_java_options[@]}" )
